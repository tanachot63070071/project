...
def TotalofUnit():
    ...
    Prices_Drinks = (Unit1 * 10) + (Unit2 * 20) + (Unit3 * 20) + (Unit4 * 25) + (Unit5 * 25) + (Unit6 * 20) + (Unit7 * 25) + (Unit8 * 25)

    Prices_Food = (Unit9 * 40) + (Unit10 * 50) + (Unit11 * 45) + (Unit12 * 40) + (Unit13 * 40) + (Unit14 * 50) + (Unit15 * 40) + (Unit16 * 45)
...
def Receipt():
    ...
    textReceipt.insert(END, 'ใบเสร็จ:\t\t\t'+Receipt_Ref.get() + '\t' + Date_of_Order.get() + '\n')
    textReceipt.insert(END, 'Unit\t\t\t\t'+"Total of Unit \n")
    textReceipt.insert(END, 'น้ำเปล่า:\t\t\t\t\t' + cocktail.get() + '\n')
    textReceipt.insert(END, 'โค้ก:\t\t\t\t\t' + iced_tea.get()+'\n')
    textReceipt.insert(END, 'สไปรท์:\t\t\t\t\t' + hot_chocolate.get()+'\n')
    textReceipt.insert(END, 'น้ำส้ม:\t\t\t\t\t' + orange_juice.get()+'\n')
    textReceipt.insert(END, 'น้ำองุ่น:\t\t\t\t\t' + milkshake.get()+'\n')
    textReceipt.insert(END, 'โอวัลติน:\t\t\t\t\t' + mountain_dew.get()+'\n')
    textReceipt.insert(END, 'กาแฟ:\t\t\t\t\t' + sting.get()+'\n')
    textReceipt.insert(END, 'ชาเขียว:\t\t\t\t\t' + cobra.get()+'\n')
    textReceipt.insert(END, 'ไก่ทอด:\t\t\t\t\t' + fried_chicken.get()+'\n')
    textReceipt.insert(END, 'ผัดกระเพรา:\t\t\t\t\t' + kare_kare.get()+'\n')
    textReceipt.insert(END, 'ส้มตำ:\t\t\t\t\t' + crispy_pata.get()+'\n')
    textReceipt.insert(END, 'ก๊วยเตี๋ยว:\t\t\t\t\t' + sinigang_baboy.get()+'\n')
    textReceipt.insert(END, 'ข้าวผัด:\t\t\t\t\t' + sinigang_hipon.get()+'\n')
    textReceipt.insert(END, 'คะน้าหมูกรอบ:\t\t\t\t\t' + bicol_express.get()+'\n')
    textReceipt.insert(END, 'ข้าวมันไก่:\t\t\t\t\t' + asparagus_tofu.get()+'\n')
    textReceipt.insert(END, 'สุกี้:\t\t\t\t\t' + chicken_sisig.get()+'\n')
    textReceipt.insert(END, 'ราคาเครื่องดื่ม:\t\t\t\t' + Total_of_Drinks.get()+'\nTax Paid:\t\t\t\t'+PaidTax.get()+"\n")
    textReceipt.insert(END, 'ราคาอาหาร:\t\t\t\t' + Total_of_Food.get()+'\nSubTotal:\t\t\t\t'+str(SubTotal.get())+"\n")
    textReceipt.insert(END, 'ค่าบริการ:\t\t\t\t' + ServiceCharge.get()+'\nTotal Cost:\t\t\t\t'+str(TotalCost.get())+"\n")


# Drinks
Cocktail = Checkbutton(Drinks_Function, text='น้ำเปล่า', variable=variable1, onvalue=1, offvalue=0, font=('arial', 16, 'bold'),
                    bg='sky blue', command=drinksCocktail).grid(row=0, sticky=W)
IcedTea = Checkbutton(Drinks_Function, text='โค้ก', variable=variable2, onvalue=1, offvalue=0, font=('arial', 16, 'bold'),
                    bg='sky blue', command=drinksIceTea).grid(row=1, sticky=W)
HotChocolate = Checkbutton(Drinks_Function, text='สไปรท์', variable=variable3, onvalue=1, offvalue=0, font=('arial', 16, 'bold'),
                    bg='sky blue', command=drinksHotChocolate).grid(row=2, sticky=W)
OrangeJuice = Checkbutton(Drinks_Function, text='น้ำส้ม', variable=variable4, onvalue=1, offvalue=0, font=('arial', 16, 'bold'),
                    bg='sky blue', command=drinksOrangeJuice).grid(row=3, sticky=W)
MilkShake = Checkbutton(Drinks_Function, text='น้ำองุ่น', variable=variable5, onvalue=1, offvalue=0, font=('arial', 16, 'bold'),
                    bg='sky blue', command=drinksMilkShake).grid(row=4, sticky=W)
MountainDew = Checkbutton(Drinks_Function, text='โอวัลติน', variable=variable6, onvalue=1, offvalue=0, font=('arial', 16, 'bold'),
                    bg='sky blue', command=drinksMountainDew).grid(row=5, sticky=W)
Sting = Checkbutton(Drinks_Function, text='กาแฟ', variable=variable7, onvalue=1, offvalue=0, font=('arial', 16, 'bold'),
                    bg='sky blue', command=drinksSting).grid(row=6, sticky=W)
Cobra = Checkbutton(Drinks_Function, text='ชาเขียว', variable=variable8, onvalue=1, offvalue=0, font=('arial', 16, 'bold'),
...
FriedChicken = Checkbutton(Food_Function,text="ไก่ทอด\t\t\t ",variable=variable9,onvalue = 1, offvalue=0,
                        font=('arial',16,'bold'),bg='sky blue',command=foodsFriedChicken).grid(row=0,sticky=W)
KareKare = Checkbutton(Food_Function,text="ผัดกระเพรา",variable=variable10,onvalue = 1, offvalue=0,
                        font=('arial',16,'bold'),bg='sky blue',command=foodsKareKare).grid(row=1,sticky=W)
CrispyPata = Checkbutton(Food_Function,text="ส้มตำ ",variable=variable11,onvalue = 1, offvalue=0,
                        font=('arial',16,'bold'),bg='sky blue',command=foodsCrispyPata).grid(row=2,sticky=W)
SinigangBaboy = Checkbutton(Food_Function,text="ก๊วยเตี๋ยว ",variable=variable12,onvalue = 1, offvalue=0,
                        font=('arial',16,'bold'),bg='sky blue',command=foodsSinigangBaboy).grid(row=3,sticky=W)
SinigangHipon = Checkbutton(Food_Function,text="ข้าวผัด ",variable=variable13,onvalue = 1, offvalue=0,
                        font=('arial',16,'bold'),bg='sky blue',command=foodsSinigangHipon).grid(row=4,sticky=W)
BicolExpress = Checkbutton(Food_Function,text="คะน้าหมูกรอบ ",variable=variable14,onvalue = 1, offvalue=0,
                        font=('arial',16,'bold'),bg='sky blue',command=foodsBicolExpress).grid(row=5,sticky=W)
AsparagusTofu = Checkbutton(Food_Function,text="ข้าวมันไก่ ",variable=variable15,onvalue = 1, offvalue=0,
                        font=('arial',16,'bold'),bg='sky blue',command=foodsAsparagusTofu).grid(row=6,sticky=W)
ChickenSisig = Checkbutton(Food_Function,text="สุกี้ ",variable=variable16,onvalue = 1, offvalue=0,
...
# ToTal Cost
lblTotalofDrinks=Label(Total_Function,font=('arial',14,'bold'),text='ราคาเครื่องดื่ม\t',bg='sky blue',fg='black',justify=CENTER)
...
lblTotalofFood=Label(Total_Function,font=('arial',14,'bold'),text='ราคาอาหาร  ',bg='sky blue',fg='black',justify=CENTER)
...
lblServiceCharge=Label(Total_Function,font=('arial',14,'bold'),text='ค่าบริการ',bg='sky blue',fg='black',justify=CENTER)
...
# BUTTONS
buttonTotal=Button(Buttons_Function,padx=16,pady=1,bd=7,fg='gold',font=('arial',12,'bold'),width=4,text='รวม',bg='black',command=TotalofUnit).grid(row=0,column=0)
buttonReceipt=Button(Buttons_Function,padx=16,pady=1,bd=7,fg='gold',font=('arial',12,'bold'),width=4,text='ใบเสร็จ',bg='black',command=Receipt).grid(row=0,column=1)
buttonReset=Button(Buttons_Function,padx=16,pady=1,bd=7,fg='gold',font=('arial',12,'bold'),width=4,text='รีเซ็ต',bg='black',command=Reset).grid(row=0,column=2)
buttonExit=Button(Buttons_Function,padx=16,pady=1,bd=7,fg='gold',font=('arial',12,'bold'),width=4,text='ออก',bg='black',command=iExit).grid(row=0,column=3)
